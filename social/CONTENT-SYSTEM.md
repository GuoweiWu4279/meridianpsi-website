...
> **★ 选题从哪来(2026-06-25 接入 GEO):** Reddit 选题不只靠"Gary 刷到的帖"。GEO 审计每周产出"AI 实际引用了哪些 Reddit 帖 / 我们在哪些买家查询上隐形 / 谁击败了我们"——这就是 Reddit 的目标清单。**这条闭环 = `../GEO-INTEGRATION.md`**(GEO↔Reddit 一个系统)。写评论前若是 GEO 目标帖,先读它的 §3 可引用性规则(单条自足、点名实体、答在前)。
...
- **`../GEO-INTEGRATION.md` — GEO↔Reddit 闭环(选题/目标的来源 + 可引用性规则);`../GEO-FINDINGS-2026-06-25.md` — 最新 GEO 实测证据**
