...
faqs:
  ...
  - q: "Can it close my open positions automatically?"
    a: "By default, no. When Guard triggers a Disconnect (L5), it stops new entries while leaving any open positions under your full control. Optionally, you can configure Guard to auto-liquidate all open positions at the moment the pause activates — this is an explicit opt-in setting. If not enabled, open positions are never touched."
  ...
  - q: "What exactly happens when Guard disconnects the broker?"
    a: "Meridian calls NinjaTrader's standard broker disconnect API — the same action as manually clicking 'Disconnect' in the NT8 interface. By default, any open positions remain at the broker; they are not closed. If you've enabled the auto-liquidation option, Guard closes all open positions before disconnecting. To reconnect, you simply reconnect through NT8 normally."
